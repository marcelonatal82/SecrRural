  ...
  class Itens extends Connect
  {
    ...
    public function listItens($idEquip, $idFabricante)
    {
      ...
      while ($rowlist = mysqli_fetch_array($result)) {
        ...
        $e = $q - $v;
        $NomeEquip = $rowlist['NomeEquip'];
        $fabricante  = $rowlist['NomeFabricante'];
      }

      return array('NomeEquip' => $NomeEquip, 'Fabricante' => $fabricante, 'QuantItens' => $q, 'QuantItensVend' => $v, 'Estoque' => $e,);
    }

    public function totalitens($value)
    {
      $this->query = "SELECT `Equip_CodRefEquip`, `Fabricante_idFabricante` FROM `itens` WHERE `itensPublic` = '$value' GROUP BY `Equip_CodRefEquip`, `Fabricante_idFabricante`";
      $this->result = mysqli_query($this->SQL, $this->query) or die(mysqli_error($this->SQL));
      while ($row = mysqli_fetch_array($this->result)) {

        $idEquip = $row['Equip_CodRefEquip'];
        $idFabricante = $row['Fabricante_idFabricante'];

        echo '<li>';
        $resp = Itens::listItens($idEquip, $idFabricante);
        echo '<b> Equipamentos: ' . $resp['NomeEquip'];
  ...
  <div>
    ...
    public function editItens($value)
    {
      ...
      if ($row = mysqli_fetch_array($this->result)) {
        ...
        $Equip_CodRefEquip = $row['Equip_CodRefEquip'];
